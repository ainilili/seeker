package com.nico.seeker.scan.impl;
...
import com.nico.seeker.dom.DomMember;
import com.nico.seeker.http.HttpMethod;
import com.nico.seeker.http.HttpUtils;
import com.nico.seeker.regex.RegexUtils;
...
public class NicoScanner extends SeekerScanner{
	...
	public List<DomBean> domScan(String dom){
		...
		int isOff  = 0;									//标签闭合标记
		DomBean domProcesser = new DomBean();			//存储Domcument
		for(int index = 0; index < domChars.length; index++){
			...
			if(member){
				...
				if((splies == 0 && dplies == 0) || (! isOther && readBody )){
					if(readBody){
						...
						if(c == '<' && domChars[index + 1] != '/'){
							...
							if(DomMember.MEMBERS_ANNOTATION.containsKey(refer)){
								refer = DomMember.MEMBERS_ANNOTATION.get(refer);
							}
				 ...
				 * 是的话开启搜索（前缀）
				 */
				if(c == '<' && !searcher && index < domChars.length - 1 && domChars[index+1] != '/' && domChars[index+1] != '?'){
					cache.setLength(0);
					searcher = true;
					isOther  = false;
					splies 	 = 0;
				 ...
				 * 		无注册: searcher => false
				 */
				if((isBlank(c) || c == '/' || c == '>') &&  searcher && !cache.toString().equals("")){
